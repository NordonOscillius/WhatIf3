...
## Пол лица, для которого используется эта фраза. Это значение будет переопределять gender-значения запросов, связанных с получением конкретной формы фамилии (но не должности). Так, при _gender_override == T00_WordGender.FEMININE фраза "лейтенант Михайлов" будет превращаться во фразу "лейтенант Михайлова".
var _gender: int
